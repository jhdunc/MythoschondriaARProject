...
namespace Broccoli.Model
{
	...
	public class BezierCurve : ISerializationCallbackReceiver {
		...
		/// </summary>
		public void SetNoise () {
			SetNoise (noiseFactorAtFirstNode, noiseFactorAtLastNode, noiseScaleAtFirstNode, noiseScaleAtLastNode, spareNoiseOffsetAtFirstPoint, noiseLengthOffset, noiseLengthScale);
		}
		/// <summary>
		...
		*/
		...
		public void ApplyTransform (float scale, Quaternion rotation) {
			...
			for (int i = 0; i < points.Count; i++) {
				...
				points [i].up = rotation * points [i].up;
