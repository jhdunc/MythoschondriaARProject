    ...
    public class SproutSubfactory {
        ...
        public void GenerateSnapshotCurve (BranchDescriptor branchDescriptor) {
            ...
            while (currentBranch != null) {
                ...
                /*
                // Get noise.
                noiseScaleAtFirstNode = currentBranch.curve.noiseFactorAtFirstNode;
                noiseFactorAtFirstNode = currentBranch.curve.noiseFactorAtFirstNode;
                noiseOffset = currentBranch.curve.noiseLengthOffset;
                spareFirstNode = currentBranch.curve.spareNoiseOffsetAtFirstPoint;
                ...
                if (currentBranch.followUp == null) {
                    ...
                    if (sproutPosition == 1f) {
                        Vector3 lastSproutPos = currentBranch.GetPointAtPosition (1f)
                            + (lastSprout.sproutDirection * lastSprout.meshHeight);
                        branchDescriptor.lastSproutPosition = lastSproutPos;
                    }
                }
                if (currentBranch.followUp == null) {
                    noiseScaleAtLastNode = currentBranch.curve.noiseScaleAtLastNode;
                    noiseFactorAtLastNode = currentBranch.curve.noiseFactorAtLastNode;
                }
                */
                isFirstBranch = false;
            ...
            branchDescriptor.curve.SetNoise (noiseFactorAtFirstNode, noiseFactorAtLastNode,
                noiseScaleAtFirstNode, noiseScaleAtLastNode, spareFirstNode, noiseOffset);
        }
        /// <summary>
        ...
        public void GenerateSnapshotPolygonsPerLOD (int lodLevel, BranchDescriptor branchDescriptor) {
            ...
            for (int fragIndex = 0; fragIndex < fragments.Count; fragIndex++) {
                ...
                sproutCompositeManager.GenerateMaterials (polygonArea, branchDescriptor, true);
            }
        ...
        public void ProcessSnapshots () {
            ...
            for (int i = 0; i < branchDescriptorCollection.branchDescriptors.Count; i++) {
                ...
                if (!sproutCompositeManager.HasSnapshot (branchDescriptor.id)) {
                    branchDescriptorIndex = i;
                    BranchDescriptorCollectionToPipeline ();
                    ProcessSnapshot (i, false);
                    ProcessSnapshotPolygons (branchDescriptor);
                }
            }
        }
        #endregion
        ...
        public void SetTextureAsNormalMap (Texture2D texture, string texturePath) {
            #if UNITY_EDITOR
